...
Conference Info:"""
        ...
        if field == "authors":
            ...
            for entry in author_entries:
                ...
                # More lenient name validation - allow single words if they look like names
                if name and (len(name.split()) >= 2 or (len(name.split()) == 1 and len(name) >= 3 and name.istitle())):
                    authors.append({
                        "name": name,
                        "affiliations": affiliations,
                        "email": None
